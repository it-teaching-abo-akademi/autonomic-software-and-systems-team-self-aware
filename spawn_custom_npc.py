	...
	try:
		...
		def try_spawn_random_vehicle_at(transform):
			...
			if vehicle is not None:
				...
				#Executor for ai_control
				executor = autopilot.get_executor()
				#Planner for ai_control
				planner = autopilot.get_planner()
							
				#Destination
				ex1 = carla.Vector3D(42.5959,-4.3443,1.8431)
				
				#autopilot.set_destination(ex1)
				planner.make_plan(vehicle.get_transform(),ex1)

				#executor.update_control(vehicle,0,0)
				print("destination (planner): " + str(planner.get_current_destination()))

				#map = world.get_map()
				#waypointlist = map.get_topology()
				#for waypoint in waypointlist:
				#	print(waypoint)

				#control.update_control(vehicle,spawn_point[0],NULL,NULL)
				#ai.executor.update_control(vehicle)
